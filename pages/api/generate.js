...
export default async function (req, res) {
  ...
  const contentFilter = await openai.createCompletion("content-filter-alpha", {
  ...
  // console.log(contentFilter);
  (contentFilter == 2) ? res.status(200).json({ result: "No results" }) : res.status(200).json({ result: completion.data.choices[0].text });
}
